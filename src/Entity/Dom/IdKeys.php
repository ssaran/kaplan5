<?php


namespace K5\Entity\Dom;


class IdKeys
{
    public string $Employer = '';
    public string $BaseCover = '';
    public string $BaseJs = '';
    public string $BaseJsCover = '';
    public string $BaseJsPhone = '';
    public string $IndexCover = '';
    public string $ListCover = '';
    public string $TriggerNew = '';
    public string $TriggerSync = '';
    public string $FormNew = '';
    public string $SubmitNew = '';
    public string $ModalNew = '';
    public string $TriggerUpdate = '';
    public string $FormUpdate = '';
    public string $SubmitUpdate = '';
    public string $ModalUpdate = '';
    public string $TriggerDelete = '';
    public string $FormDelete = '';
    public string $SubmitDelete = '';
    public string $ModalDelete = '';
    public string $ModalApp = '';
    public string $DataTable = '';
    public string $DataStorage = '';
    public string $FormChangePassword = '';
    public string $SearchInput = "";
    public string $SearchButton = "";
    public string $SearchResult = '';
    public string $PaginatorTableCover = '';
    public string $PaginatorTotalCover = '';
    public string $CloseSideBar = '';
    public string $password = '';
    public string $password_repeat = '';
    public string $ExportToExcel = '';

    public string $City = '';
    public string $Town = '';
    public string $District = '';
    public string $Neighborhood = '';
    public string $PostCode = '';
    public string $url = '';
    public string $ReloadTrigger = '';
    public string $AnchorHOT = '';
    public string $CoverHOT = '';
    public string $PrintIframe = '';
    public string $CustomerCallRecord = '';

    public string $ModalTuncaPhoneSetup = '';
    public string $BtnTuncaPhoneSetupTrigger = '';
    public string $TuncaPhone = '';

    public string $BaseTab = '';

}
